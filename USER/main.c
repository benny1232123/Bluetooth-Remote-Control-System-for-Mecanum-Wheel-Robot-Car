...
#define BT_LIFT_UP_CLK 2500
#define BT_LIFT_DOWN_CLK 2500
...
static void BT_ApplyCommand(uint8_t raw_cmd)
{
	...
	switch (cmd)
	{
		...
		case 'U': // lift up
			if (is_press)
			{
				Emm_V5_Pos_Control(USART2, 1, 8, BT_LIFT_VEL, BT_LIFT_ACC, BT_LIFT_UP_CLK, 0, 0);
			}
			else
			{
		...
		case 'N': // lift down
			if (is_press)
			{
				Emm_V5_Pos_Control(USART2, 1, 0, BT_LIFT_VEL, BT_LIFT_ACC, BT_LIFT_DOWN_CLK, 0, 0);
			}
			else
			{
				Emm_V5_Stop_Now(USART2, 1, 0);
