			  ...
			  { 	LCD_SetWindows(x,y,x+64-1,y+64-1);
				    for(i=0;i < 16*32;i++)
				    {
						for(j=0;j<8;j++)
				    	{
							if(!mode) //非叠加方式
							{
								if(tfont64[k].Msk[i]&(0x80>>j))	Lcd_WriteData_16Bit(fc);
								else Lcd_WriteData_16Bit(bc);
							}
							else
							{
...
void GUI_DrawFont64_one(u16 x, u16 y, u16 fc, u16 bc, u8 k,u8 mode)
{
	u16 i,j;
	u16 x0=x;
	LCD_SetWindows(x,y,x+64-1,y+64-1);
...
void GUI_DrawFont40_80_one(u16 x, u16 y, u16 fc, u16 bc, u8 k,u8 mode)
{
	u16 i,j;
	u16 x0=x;
	LCD_SetWindows(x,y,x+40-1,y+80-1);
	for(i=0;i < 16*25;i++)
	{
	for(j=0;j<8;j++)
		{
			if(!mode) //非叠加方式
			{
				if(tfont40_80[k].Msk[i]&(0x80>>j))	Lcd_WriteData_16Bit(fc);
				else Lcd_WriteData_16Bit(bc);
			}
			else
			{
				POINT_COLOR=fc;
				if(tfont40_80[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y);//画一个点
				x++;
				if((x-x0)==40)
				{
					x=x0;
...
void GUI_DrawFont48_one(u16 x, u16 y, u16 fc, u16 bc, u8 k,u8 mode)
{
	u16 i,j;
	u16 x0=x;
